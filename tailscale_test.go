...
func TestValidateTailscaleAuth(t *testing.T) {
	base := func() *Config {
		return &Config{
			...
			// Set so Validate() reaches the auth checks rather than stopping at
			// the empty-required_tags guard, which is what is under test here.
			App:     AppConfig{RequiredTags: []string{"tag:dns"}},
